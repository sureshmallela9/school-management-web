  ...
  template: `
    <section class="page-shell">
      ...
      <section class="panel" *ngIf="activeTab === 'ledger'">
        ...
        <div class="filter-row"><label>Status<select [(ngModel)]="ledgerStatus" name="ledgerStatus" (ngModelChange)="loadLedger()"><option value="">All</option><option>PENDING</option><option>PARTIAL</option><option>PAID</option></select></label><label>Class<select [(ngModel)]="ledgerClassId" name="ledgerClassId" (ngModelChange)="loadLedger()"><option value="">All classes</option><option *ngFor="let cls of lookup.classOptions()" [value]="cls.id">{{ cls.label }}</option></select></label></div>
      ...
      <section class="panel" *ngIf="activeTab === 'assignments'">
        ...
        <form class="form-grid" (ngSubmit)="assignFee()"><label>Fee structure<select [(ngModel)]="assignmentForm.feeStructureId" name="assignmentStructure" required><option value="">Select structure</option><option *ngFor="let structure of lookup.structureOptions()" [value]="structure.id">{{ structure.label }}</option></select></label><label>Student<select [(ngModel)]="assignmentForm.studentId" name="assignmentStudent" required><option value="">Select student</option><option *ngFor="let student of lookup.studentOptions()" [value]="student.id">{{ student.label }}</option></select></label><label>Override amount<input type="number" min="0.01" step="0.01" [(ngModel)]="assignmentForm.overrideAmount" name="assignmentAmount" /></label></form>
        ...
        <form class="form-grid" (ngSubmit)="assignClassFee()"><label>Fee structure<select [(ngModel)]="classAssignmentForm.feeStructureId" name="classAssignmentStructure" required><option value="">Select structure</option><option *ngFor="let structure of lookup.structureOptions()" [value]="structure.id">{{ structure.label }}</option></select></label><label>Class<select [(ngModel)]="classAssignmentForm.classId" name="classAssignmentClass" required><option value="">Select class</option><option *ngFor="let cls of lookup.classOptions()" [value]="cls.id">{{ cls.label }}</option></select></label><label>Override amount<input type="number" min="0.01" step="0.01" [(ngModel)]="classAssignmentForm.overrideAmount" name="classAssignmentAmount" /></label></form>
